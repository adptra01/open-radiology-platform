...
| **M11** | **In-progress (2026-09-19)** | Sisa big-bang: image ai-worker rebuild+verified, legacy `orp_ai`+card dihapus (xrv via dispatcher, test 46 hijau), harness evaluasi Phase 3 siap. Sisa: bobot real (Colab retry gagal, Kaggle manual) + kalibrasi + auditability. Out-of-scope tetap: model CT/MR/US, LLM report, auto-diagnosis |
| **M12** | **In-progress — M12.1+M12.2 selesai (2026-09-20)** | B1 otorisasi reports.* + B4 FINAL immutable/amendment-record/awaitingReport-FINAL-only; B2 STOW via PacsClient+auth; B3 C-STORE 0xA700; B5 storage reference + volume `orp-dicom-inbox` (dev+prod+ddev). Suite 145+46+16 hijau. Lanjut M12.3 (Patient+Order) |
